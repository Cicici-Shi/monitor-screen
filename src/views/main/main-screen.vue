    ...
    <FixedContent>
      <div class="screen-container">
        <div class="item-1">
          ...
          <div class="nav-r">
            <ul>
              <li v-for="n in headList" :key="n.key">
                {{n.name}}
                <b>
                  <NumberGrow :value="n.render(result.headList).value"></NumberGrow>
            ...
            <ul><li>数据统计范围：{{result.currentTime}}</li></ul>
          </div>
        </div>
        <div class="item-2">
          <div class="title-panel">数据库概览</div>
          <ul>
            <li v-for="(n, i) in result.databaseOverview" :key="n.title">
              <b>{{n.title}}</b>
              <div class="echarts-box">
                <div :id="'overview'+ i" style="width:100%; height:120px"></div>
                <div class="center">
                  <b>
                    <NumberGrow :value="overviewTotal(n)" :decimals="0"></NumberGrow>
                  </b>
                  <p>数据库</p>
                </div>
              </div>
              <div class="hint">
                <p v-for="(o, j) in n.list" :data-color='o.name' :key="i+j">{{o.name}}：{{o.count}}</p>
              </div>
            </li>
          </ul>
        </div>
        <div class="item-8">
          <div class="box">
            <div class="max-title">数据同步延迟监控</div>

            <div>
              <svg xmlns="http://www.w3.org/2000/svg" width="50%" height="50%" viewBox="0 0 400 400">
                <circle cx="106" cy="200" r="71" stroke="#3dcc9d"
  stroke-width="7" fill="#0a1c47" stroke-dasharray="35 5 20 15 5 5"/>
                <circle cx="106" cy="200" r="21" stroke="#3dcc9d"
  stroke-width="7" fill="#3dcc9d" stroke-dasharray="5"/>
              </svg>
            </div>
            <div class="monitor-border"></div>
            <ul class="monitoring-box" :style="{'--length': synMonitoringLength}">
              <li v-for="(n, i) in result.synMonitoring" :key='n.db_name' :class="{on: i === currentObj.currentItem}" :style="{'--index': i}">
                <span>
                  <div class="cylinder" :class="{abnormal: n.syn_status === 2}"></div>
                </span>
                <b></b>
              </li>
            </ul>
            <div class="status-container">
              <div class="state">同步状态占比</div>
              <div class="total" data-status="同步正常">
                <span>同步正常</span>
                <p>
                  {{synMonitoringLength -  synMonitoringNormalLeng}}个
                </p>
              </div>
              <div class="total" data-status="同步延迟">
                <span>同步延迟</span>
                <p>
                  {{synMonitoringNormalLeng}}个
                </p>
              </div>
            </div>
            <div class="reminder-container">
              <vue3-seamless-scroll :list="result.synMonitoring" :step="0.2" :limitScrollNum="5">
                <ul>
                  <li v-for="n in result.synMonitoring" :key="n">{{n.desc}}:&nbsp;延迟{{n.delay_time}}</li>
                </ul>
        ...
        <div class="item-9">
          <div class="title-panel">厂商调用数量TOP5</div>
          <div class="box">
            <ul>
              <li data-key="TOP4">
                <div class="prompt">
              ...
              <li data-key="TOP2">
                <div class="prompt">
                  {{unitConverter(result.top5[1].rowCount).value}}{{unitConverter(result.top5[1].rowCount).unit}}
        ...
        <div class="item-3">
          <div class="title-panel">队列资源监控</div>
          <div class="box">
            <div class="queue">{{result.queueList.scheduler.schedulerInfo.queueName}} {{result.queueList.scheduler.schedulerInfo.used}}% 使用中</div>
            <div class="monitoring-list">
              <vue3-seamless-scroll :list="result.queueList.scheduler.schedulerInfo.queues.queue" :step="0.1" :limitScrollNum="8">
                <ul>
                  <li v-for="o in result.queueList.scheduler.schedulerInfo.queues.queue" :key="o.queueName" :class="calcRatio(o)['cls']">
                    <span class="l">{{o.queueName}}</span>
                    <span class="c">
                      <div class="bar" :style="calcRatio(o)['style']"></div>
                    </span>
        ...
        <div class="item-4">
          <div class="title-panel">数据库API调用热度TOP10</div>
          <div class="boxs">
            <vue3-seamless-scroll :list="result.databaseTop10" :step="0.2" :limitScrollNum="5">
              <ul>
                <li v-for="n in result.databaseTop10" :key="n.databaseName">
                  <div class="l">
                    <p> {{n.databaseName}}</p>
                    <p> {{n.databaseRemark || '暂无说明'}}</p>
                  </div>
                  <div class="r">
                    <div class="text">{{unitConverter(n.rowCount, 2).value}}{{unitConverter(n.rowCount, 2).unit}}</div>
                    <span>
                      <div class="bar" :style="{'width': n.rowCount / result.databaseTop10[0].rowCount * 100 + '%'}"></div>
                    </span>
                  </div>
                </li>
              </ul>
        ...
        <div class="item-5">
          <div class="title-panel">延迟概览</div>
          <div class="boxs">
            ...
            <ul>
              <li class="first">
                <div data-status="同步正常">
                  <p>
                    <NumberGrow :value="result.delayOverview[2].normal" :decimals="0"></NumberGrow>
                  </p>
                  <span>同步正常</span>
                </div>
                <div data-status="同步延迟">
                  <p>
                    <NumberGrow :value="result.delayOverview[2].abnormal" :decimals="0"></NumberGrow>
                  </p>
                  <span>同步延迟</span>
                </div>
                <div>
                  <div id="total" style="width:80px;height:70px">
                    <ProgressCircle :rate="result.delayOverview[2].normal / result.delayOverview[2].total" ring-color="#3dcc9d" bgRingColor="#596080" stroke-linecap="round" :stroke-width="30">
        ...
        <div class="item-6">
          <div class="title-panel">数据同步延迟</div>
          <div class="boxs">
            <div id="synDelayEcharts" style="width:100%;height:240px"></div>
          </div>
        </div>
        <div class="item-7">
          <div class="title-panel">数据表API调用热度Top10</div>
          <div class="boxs">
            <vue3-seamless-scroll :list="result.tableTop10" :step="0.2" :limitScrollNum="5">
              <ul>
                <li v-for="n in result.tableTop10" :key="n.tableName">
                  <div class="l">
                    <p> {{n.tableName}}</p>
                  ...
                  <div class="r">
                    <div class="text">{{unitConverter(n.rowCount, 2).value}}{{unitConverter(n.rowCount, 2).unit}}</div>
                    <span>
                      <div class="bar" :style="{'width': n.rowCount / result.tableTop10[0].rowCount * 100 + '%'}"></div>
                    </span>
                  </div>
                </li>
              </ul>
...
const databaseOverviewEcharts = () => {
  let chartDom = [];
  let myChart = [];
  let bgColor = [['#3dcc9d', '#f5983b'], ['#3dcc9d', '#65d0e5', '#f5983b'], ['#65d0e5', '#8585ff', '#898ca3']];
...
const result = reactive({
  ...
  databaseOverview: [
    {
      title: 'T0数据库',
      list: [{name: '同步正常', count: 21}, {name: '同步异常', count: 1}]
    },
    {
      title: 'TN数据库',
      list: [{name: '更新正常', count: 37}, {name: '正在更新', count: 0}, {name: '更新异常', count: 23}]
    },
    {
      title: '数据更新方式',
      list: [{name: 'T0数据库', count: 22}, {name: 'TN数据库', count: 60}, {name: '历史数据库', count: 50}]
    },
  ],
  queueList: {
    scheduler: {
    ...
    ul {
      ...
      li {
        ...
        .hint {
          ...
          p[data-color='更新正常'], p[data-color='同步正常'] {
            color: #3dcc9d
          }
          p[data-color='正在更新'], p[data-color='T0数据库'] {
            color: #65d0e5
          }
          p[data-color='更新异常'], p[data-color='同步异常'] {
            color: #f7ad63
          }
        }
      }
        ...
        li {
          ...
          span {
            ...
            &.c {
              ...
              .bar {
                ...
                height: 8px;
                width: 10%;
                background-image: linear-gradient(to right, rgba(255, 255, 255, 19%), #60dbb3);
              }
            }
            &.r {
              margin-left: 5px;
          ...
          &.used-over-capacity {
            span {
              ...
              &.c {
                .bar {
                  background-image: linear-gradient(to right, rgba(255, 255, 255, 19%), #f58718);
                }
              }
            }
          }
...
// }
      ...
      .reminder-container, .operation-container {
        width: 200px;
        background: #0a1c47;
